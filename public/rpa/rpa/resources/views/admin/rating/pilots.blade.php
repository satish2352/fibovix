						...
						<table class="table table-striped" id="pageDataTable" role="grid" aria-describedby="DataTables_Table_0_info">
							...
							<tbody>
								...
								<tr>
									...
									<td><?php  $ftoDetails=$CommonService->getFTODetails($getallPilots->ftoId); ?>{{ $ftoDetails[0]->firstName}} {{$ftoDetails[0]->middleName}} {{$ftoDetails[0]->lastName }}</td>
									...
									<td>
								        @if ($getallPilots->ftoApproved == 1)
                                                ...
                                                <a href="{{route('admpilotrating',array('ftotId'=>$getallPilots->pilotIdFInal,'rating'=>'1'))}}" title="1 Star" >
                                                  <i class="fa fa-star @if($getallPilots->rating) @if(($getallPilots->rating)>=1) {{'rated'}} @endif @else {{'rating'}} @endif" ></i>
                                                </a>
                                                
                                                <a href="{{route('admpilotrating',array('ftotId'=>$getallPilots->pilotIdFInal,'rating'=>'2'))}}" title="2 Star" >
                                                 <i class="fa fa-star rating  @if($getallPilots->rating) @if(($getallPilots->rating)>=2) {{'rated'}} @endif @else {{'rating'}} @endif" ></i>
                                                </a>
                                            
                                                <a href="{{route('admpilotrating',array('ftotId'=>$getallPilots->pilotIdFInal,'rating'=>'3'))}}" title="3 Star"> 
                                                 <i class="fa fa-star rating @if($getallPilots->rating) @if(($getallPilots->rating)>=3) {{'rated'}} @endif @else {{'rating'}} @endif"></i>
                                                </a>
                                            
                                                <a href="{{route('admpilotrating',array('ftotId'=>$getallPilots->pilotIdFInal,'rating'=>'4'))}}" title="4 Star" >
                                                 <i class="fa fa-star rating @if($getallPilots->rating) @if(($getallPilots->rating)>=4) {{'rated'}} @endif @else {{'rating'}} @endif"></i>
                                                </a>
                                            
                                                <a href="{{route('admpilotrating',array('ftotId'=>$getallPilots->pilotIdFInal,'rating'=>'5'))}}" title="5 Star" >
