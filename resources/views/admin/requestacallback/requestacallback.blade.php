			...
			<div class="min-height-200px">
						...
						<div class="col-md-6 col-sm-12">
							...
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
									...
									<li class="breadcrumb-item active" aria-current="page">Request A Callback</li>
								</ol>
							</nav>
						</div>
				...
				<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
					<div class="clearfix mb-20">
						<div class="pull-left">
							<h5 class="text-blue">Request A Callback List</h5>
						</div>
					</div>
					<?php
					...
					<div class="row table-responsive ">
						...
						<table class="table table-striped" id="pageDataTable" role="grid" aria-describedby="DataTables_Table_0_info">
							...
							<tbody>
							    ...
							    @if(count($requestACallbackList) > 0)
							    <?php
							    
                                    foreach($requestACallbackList as $requestACallbackList )
                                      {
                                ?>
								<tr>
								    <td>{{ ++$count}}</td>
								    <td>{{ $requestACallbackList->name}}</td>
									<td>{{ $requestACallbackList->email }}</td>
									<td>{{ $requestACallbackList->mobileNumber }}</td>
									<td>{{ $requestACallbackList->description }}</td>
								</tr>
								<?php
                                          }
                                          ?>
