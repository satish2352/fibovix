					...
					<div class="row table-responsive ">
                     	...
                     	<table class="table table-striped" id="pageDataTable" role="grid" aria-describedby="DataTables_Table_0_info">
							...
							<tbody>
                                ...
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td><?php  $ftoDetails=$CommonService->getFTODetails($value->underFTO); ?>{{ $ftoDetails[0]->firstName}} {{$ftoDetails[0]->middleName}} {{$ftoDetails[0]->lastName }}</td>
                                    <td>{{$value->instructorName}}</td>
                                    <td>@if($value->status==0) {{'Active'}} @else {{'Not Active'}} @endif</td>
                                    <td>
								      
								            
                                                <a href="{{route('adminstructorrating',array('ftotId'=>$value->id,'rating'=>'1'))}}" title="1 Star" >
                                                  <i class="fa fa-star @if($value->rating) @if(($value->rating)>=1) {{'rated'}} @endif @else {{'rating'}} @endif" ></i>
                                                </a>
                                                
                                                <a href="{{route('adminstructorrating',array('ftotId'=>$value->id,'rating'=>'2'))}}" title="2 Star" >
                                                 <i class="fa fa-star rating  @if($value->rating) @if(($value->rating)>=2) {{'rated'}} @endif @else {{'rating'}} @endif" ></i>
                                                </a>
                                            
                                                <a href="{{route('adminstructorrating',array('ftotId'=>$value->id,'rating'=>'3'))}}" title="3 Star"> 
                                                 <i class="fa fa-star rating @if($value->rating) @if(($value->rating)>=3) {{'rated'}} @endif @else {{'rating'}} @endif"></i>
                                                </a>
                                            
                                                <a href="{{route('adminstructorrating',array('ftotId'=>$value->id,'rating'=>'4'))}}" title="4 Star" >
                                                 <i class="fa fa-star rating @if($value->rating) @if(($value->rating)>=4) {{'rated'}} @endif @else {{'rating'}} @endif"></i>
                                                </a>
                                            
                                                <a href="{{route('adminstructorrating',array('ftotId'=>$value->id,'rating'=>'5'))}}" title="5 Star" >
