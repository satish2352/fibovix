						...
						<table class="table table-striped" id="pageDataTable" role="grid" aria-describedby="DataTables_Table_0_info">
							...
							<tbody>
								...
								<tr>
									...
									<td>
								        @if ($getallBlog->blogBy == "0")
								            <span>Pilot</span>
								        @elseif ($getallBlog->blogBy == "1")
								            <span>FTO</span>
								        @else
								            <span>Admin</span>
                                        @endif
									...
									<td>
								        @if ($getallBlog->status == "1")
								            <a class="btn btn-success" href="{{route('blogshowhidebyadmin',array('id'=>$getallBlog->id,'approvalVal'=>$getallBlog->status))}}" role="button" >
									            <i class="fa fa-thumbs-up"></i>
								            </a>
								            
								        @else
								            <a class="btn btn-danger" href="{{route('blogshowhidebyadmin',array('id'=>$getallBlog->id,'approvalVal'=>$getallBlog->status))}}" role="button" >
								    ...
								    <td>
								        @if ($getallBlog->blogBy != "2")
								        
								        @if ($getallBlog->adminStatus == "1")
								            <a class="btn btn-success" href="{{route('blogapprovalbyadmin',array('id'=>$getallBlog->id,'approvalVal'=>$getallBlog->adminStatus))}}" role="button" >
									            <i class="fa fa-thumbs-up"></i>
								            </a>
								            
								        @else
								            <a class="btn btn-danger" href="{{route('blogapprovalbyadmin',array('id'=>$getallBlog->id,'approvalVal'=>$getallBlog->adminStatus))}}" role="button" >
									            <i class="fa fa-thumbs-down"></i>
								            </a>
                                        @endif
                                        @endif
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        	<!--<form method="post" enctype="multipart/form-data">-->
        	    <form action="{{route('saveadminblog')}}" method="POST" enctype="multipart/form-data">
					    @csrf
